...
const sendVerificationEmail = async (email, token) => {
  const verificationUrl = `${process.env.FRONTEND_URL || 'https://delhi-aqi-eta.vercel.app'}/verify-email?token=${token}`;
...
const sendPasswordResetEmail = async (email, token) => {
  const resetUrl = `${process.env.FRONTEND_URL || 'https://delhi-aqi-eta.vercel.app'}/reset-password?token=${token}`;
